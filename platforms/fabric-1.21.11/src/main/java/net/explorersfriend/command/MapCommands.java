...
public final class MapCommands {
    ...
    public static void register() {
        CommandRegistrationCallback.EVENT.register(MapCommands::registerTree);
    }
        ...
        dispatcher.register(CommandManager.literal("efmap")
                ...
                .then(CommandManager.literal("render")
                        ...
                        .then(CommandManager.argument("dimension", DimensionArgumentType.dimension())
                                ...
                                .then(CommandManager.argument("radius", IntegerArgumentType.integer(64, 30_000_000))
                ...
                .then(CommandManager.literal("pause")
                        .requires(source -> net.explorersfriend.command.Perms.atLeast(source, 2)).executes(MapCommands::pause))
                .then(CommandManager.literal("resume")
                        .requires(source -> net.explorersfriend.command.Perms.atLeast(source, 2)).executes(MapCommands::resume))
                ...
                .then(CommandManager.literal("reload")
                        .requires(source -> net.explorersfriend.command.Perms.atLeast(source, 2)).executes(MapCommands::reload))
                .then(CommandManager.literal("scan")
                        .requires(source -> net.explorersfriend.command.Perms.atLeast(source, 2))
                        .then(CommandManager.literal("status").executes(MapCommands::scanStatus))
                        .then(CommandManager.literal("mods").executes(MapCommands::scanMods)))
                .then(CommandManager.literal("colors")
                        .requires(source -> net.explorersfriend.command.Perms.atLeast(source, 2))
                        .then(CommandManager.literal("reload").executes(MapCommands::colorsReload)))
                .then(CommandManager.literal("cache")
                        .requires(source -> net.explorersfriend.command.Perms.atLeast(source, 2))
                        .then(CommandManager.literal("stats").executes(MapCommands::cacheStats))
                        .then(CommandManager.literal("prune")
                                .requires(source -> net.explorersfriend.command.Perms.atLeast(source, 3))
                                .executes(context -> {
                                ...
                                .then(CommandManager.literal("confirm")
                                        ...
                                        .then(CommandManager.literal("tiles")
                                                .executes(context -> prune(context, true))))))
                .then(CommandManager.literal("web")
                        .requires(source -> net.explorersfriend.command.Perms.atLeast(source, 2))
                        .then(CommandManager.literal("status").executes(MapCommands::webStatus))
                        .then(CommandManager.literal("restart").executes(MapCommands::webRestart))));
    }
    ...
    private static int render(CommandContext<ServerCommandSource> context, int radius)
    ...
    private static int cancel(CommandContext<ServerCommandSource> context, ServerWorld worldOrNull) {
        MapService service = ready(context);
        if (service == null) {
            return 0;
        }
        String slug = worldOrNull == null ? null
                : TileStore.dimensionSlug(worldOrNull.getRegistryKey().getValue().toString());
    ...
    private static int scanStatus(CommandContext<ServerCommandSource> context) {
        MapService service = ready(context);
        if (service == null) {
            return 0;
        }
    ...
    private static int scanMods(CommandContext<ServerCommandSource> context) {
        MapService service = ready(context);
        if (service == null) {
            return 0;
        }
    ...
    private static int colorsReload(CommandContext<ServerCommandSource> context) {
        MapService service = ready(context);
        if (service == null) {
            return 0;
        }
    ...
    private static int cacheStats(CommandContext<ServerCommandSource> context) {
        MapService service = ready(context);
        if (service == null) {
            return 0;
        }
        ...
        Thread counter = new Thread(() -> {
            ...
            server.execute(() -> source.sendFeedback(() -> Text.literal(message), false));
        }, "EF-CacheStats");
    ...
    private static int prune(CommandContext<ServerCommandSource> context, boolean alsoTiles) {
        MapService service = ready(context);
        if (service == null) {
            return 0;
        }
    ...
    private static int webRestart(CommandContext<ServerCommandSource> context) {
        MapService service = ready(context);
        if (service == null) {
            return 0;
        }
    ...
    private static MapService ready(CommandContext<ServerCommandSource> context) {
        MapService service = MapService.get();
        if (service == null) {
            feedback(context, "Map service is not running.");
            return null;
    ...
    private static Consumer<String> asyncFeedback(CommandContext<ServerCommandSource> context) {
        ServerCommandSource source = context.getSource();
        var server = source.getServer();
        return message -> server.execute(() -> source.sendFeedback(() -> Text.literal(message), false));
    }
}
