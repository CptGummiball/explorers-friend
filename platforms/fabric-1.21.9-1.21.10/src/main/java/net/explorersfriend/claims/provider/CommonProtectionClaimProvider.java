...
public final class CommonProtectionClaimProvider implements ClaimProvider {
    ...
    private record Probe(ServerWorld world, int chunkX, int chunkZ) {
    }
    ...
    public static void chunkLoaded(ServerWorld world, int chunkX, int chunkZ) {
    ...
    private void tick() {
        ...
        while (budget-- > 0 && (probe = queue.poll()) != null) {
            ...
            String dimensionId = probe.world().getRegistryKey().getValue().toString();
    ...
    private static final class Api {
        ...
        static boolean isAreaProtected(ServerWorld world, int chunkX, int chunkZ) {
            int minX = chunkX << 4;
            int minZ = chunkZ << 4;
            Box box = new Box(minX, -2048, minZ, minX + 16, 2048, minZ + 16);
