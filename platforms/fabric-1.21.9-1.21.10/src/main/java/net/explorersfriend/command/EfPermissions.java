...
public final class EfPermissions {
    ...
    public static Predicate<ServerCommandSource> require(String node, int fallbackLevel) {
        return source -> check(source, node, fallbackLevel);
    }

    public static boolean check(ServerCommandSource source, String node, int fallbackLevel) {
        if (API_PRESENT) {
            try {
    ...
    private static final class Api {
        static boolean check(ServerCommandSource source, String node, int fallbackLevel) {
