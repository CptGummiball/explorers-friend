    ...
    private static final class Api {
        static List<WaystonePoint> collect(MinecraftServer server, MapConfig.Waystones config) {
            ...
            net.blay09.mods.waystones.api.WaystonesAPI.getAllWaystones(server).forEach(ws -> {
                ...
                String dimensionId = ws.getDimension().getValue().toString();
                ...
                String owner = null;
                if (config.showOwner()) {
                    java.util.UUID uid = ws.getOwnerUid().orElse(null);
                    if (uid != null) {
                        owner = server.getApiServices().nameToIdCache().getByUuid(uid)
                                .map(net.minecraft.server.PlayerConfigEntry::name).orElse(null);
                    }
                }
