...
public final class FtbChunksClaimProvider implements ClaimProvider {
    ...
    public List<RawArea> copyRawClaims() {
        ...
        for (ClaimedChunkAccess chunk : FTBChunksAPI.api().getManager().getAllClaimedChunks()) {
            ...
            String key = team.getTeamId() + "|" + pos.dimension().identifier();
    ...
    private static String ownerName(MinecraftServer server, UUID owner, String fallback) {
        ...
        return server.getProfileCache() == null ? fallback
                : server.getProfileCache().getByUuid(owner)
