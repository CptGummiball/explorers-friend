 ...
 * positions stay separate (position-keyed ids). ChunkAccess re-loads cause no duplicates
...
public final class BannerWatcher {
    ...
    private final Function<ServerLevel, String> slugOf;

    private final ArrayDeque<long[]> queue = new ArrayDeque<>(); // {posLong, worldIdentityHash} guarded by this
    private final Set<Long> queued = new HashSet<>();
    private final java.util.Map<Long, ServerLevel> queuedWorlds = new java.util.HashMap<>();

    public BannerWatcher(MarkerStore store, MapConfig.Markers config, Function<ServerLevel, String> slugOf) {
        this.store = store;
        this.config = config;
        this.slugOf = slugOf;
    }
    ...
    public void onBlockChanged(ServerLevel world, BlockPos pos, BlockState newState) {
    ...
    private synchronized void enqueue(ServerLevel world, BlockPos pos) {
    ...
    public void onEndTick() {
        for (int i = 0; i < MAX_CHECKS_PER_TICK; i++) {
            ...
            if (world != null) {
                checkBanner(world, BlockPos.of(entry[0]));
            }
        }
    }
     ...
     */
    public void checkBanner(ServerLevel world, BlockPos pos) {
        net.minecraft.world.level.chunk.ChunkAccess chunk = world.getChunk(pos.getX() >> 4, pos.getZ() >> 4,
     ...
     */
    public void onChunkLoaded(ServerLevel world, net.minecraft.world.level.chunk.LevelChunk chunk,
        ...
        for (net.minecraft.world.level.block.entity.BlockEntity blockEntity : chunk.getBlockEntities().values()) {
            if (blockEntity instanceof BannerBlockEntity banner && banner.getCustomName() != null) {
                checkBannerInChunk(world, chunk, blockEntity.getBlockPos());
            }
        }
    }

    /** Chunk-local verification: named banner → upsert, otherwise remove. Never touches the world lookup path. */
    void checkBannerInChunk(ServerLevel world, net.minecraft.world.level.chunk.LevelChunk chunk, BlockPos pos) {
        String slug = slugOf.apply(world);
        if (slug == null) {
            return;
        }
        ...
        Component customName = banner.getCustomName();
        if (customName == null) {
     ...
     */
    public static String encodeDesign(DyeColor base, BannerPatternLayers patterns) {
