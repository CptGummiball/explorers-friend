 ...
 * {@code BlockPos.MutableBlockPos}, flat array lookups via {@link StateColorTable}, short
...
public final class LiveChunkExtractor {
    ...
    public TileChunkData extract(ServerLevel world, LevelChunk chunk) {
        ...
        for (int z = 0; z < 16; z++) {
            for (int x = 0; x < 16; x++) {
                int top = heightmap.getFirstAvailable(x, z) - 1;
                if (top < minY) {
                    continue;
                }
                extractColumn(world, chunk, biomeRegistry, pos,
                        chunkPos.getMinBlockX() + x, chunkPos.getMinBlockZ() + z,
    ...
    private void extractColumn(ServerLevel world, LevelChunk chunk, Registry<Biome> biomeRegistry,
                               BlockPos.MutableBlockPos pos, int blockX, int blockZ, int top, int minY,
        ...
        while (y >= minY) {
            ...
            boolean waterHere = !fluid.isEmpty() && fluid.is(FluidTags.WATER);
            ...
            if (waterHere) {
                ...
                waterDepth++;
                boolean pureFluidBlock = state.getBlock() == net.minecraft.world.level.block.Blocks.WATER;
            ...
            int biomeRawId = biomeRegistry.getId(world.getBiome(pos).value());
        ...
        if (waterDepth > 0 || haveTranslucent) {
            int biomeRawId = biomeRegistry.getId(
    ...
    private int[] borderHeights(ServerLevel world, int chunkX, int chunkZ, boolean eastEdge) {
        ChunkAccess neighbour = world.getChunk(chunkX, chunkZ, ChunkStatus.FULL, false);
        if (neighbour == null) {
            return null;
        }
        Heightmap heightmap = neighbour.getOrCreateHeightmapUnprimed(Heightmap.Types.WORLD_SURFACE);
        int[] out = new int[16];
        for (int i = 0; i < 16; i++) {
            out[i] = eastEdge ? heightmap.getFirstAvailable(15, i) - 1 : heightmap.getFirstAvailable(i, 15) - 1;
        }
        return out;
    }
}
