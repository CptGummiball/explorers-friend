...
public final class ClaimManager {
    ...
    public void start(net.minecraft.server.MinecraftServer server, ScheduledExecutorService sched) {
        for (ClaimProvider provider : providers) {
            try {
                provider.subscribe(server, () -> eventDirtySince.putIfAbsent(provider.providerId(),
                        System.nanoTime()));
            } catch (Exception e) {
                LOGGER.warn("[ExplorersFriend/Claims] Could not subscribe to {} events: {}",
                        provider.providerId(), e.toString());
            }
            refresh(server, provider); // initial full sync
        }
        sched.scheduleWithFixedDelay(() -> tick(server), 5, 5, TimeUnit.SECONDS);
    }
    ...
    private void refresh(net.minecraft.server.MinecraftServer server, ClaimProvider provider) {
        String id = provider.providerId();
        ...
        CompletableFuture
                .supplyAsync(() -> {
                    if (!provider.isAvailable(server)) {
                        return List.<ClaimProvider.RawArea>of();
                    }
                    return provider.copyRawClaims(server);
                }, serverExecutor)
