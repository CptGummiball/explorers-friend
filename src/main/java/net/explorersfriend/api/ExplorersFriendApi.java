...
public final class ExplorersFriendApi {
    ...
    /**
     * Vetoes player visibility on the web map (vanish systems, permission mods, …).
     * Called on the server thread during the periodic sample — keep it cheap. If ANY
     * registered provider returns false, the player is never shown; the built-in
     * invisibility/spectator/config rules always apply additionally.
     */
    @FunctionalInterface
    public interface PlayerVisibilityProvider {
        boolean shouldDisplay(net.minecraft.server.network.ServerPlayerEntity player);
    }

    private static final CopyOnWriteArrayList<PlayerVisibilityProvider> VISIBILITY_PROVIDERS =
            new CopyOnWriteArrayList<>();

    public static void registerPlayerVisibilityProvider(PlayerVisibilityProvider provider) {
        VISIBILITY_PROVIDERS.add(provider);
    }

    public static List<PlayerVisibilityProvider> playerVisibilityProviders() {
        return VISIBILITY_PROVIDERS;
    }
    ...
    public static void fireScanComplete(int totalBlocks, int fallbackBlocks) {
        for (ScanCompleteListener listener : SCAN_LISTENERS) {
            try {
                ...
                net.explorersfriend.ExplorersFriend.LOGGER
